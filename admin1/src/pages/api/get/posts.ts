...
import { TGetPostParams, TPostContentParams, TSessionToken } from "@/types/mainSchema";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { page, limit, sortBy, sortOrder, search, category, isPublished, isPinned, author, symbol }: TGetPostParams = req.query;
    const params = { page, limit, sortBy, sortOrder, search, category, isPublished, isPinned, author, symbol };

    const config = getRequestConfigWithParams(req, session.token, params);
    const response = await apiServices.getPosts({ config });
